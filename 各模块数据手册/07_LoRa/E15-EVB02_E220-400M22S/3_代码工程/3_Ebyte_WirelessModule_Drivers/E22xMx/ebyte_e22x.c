...
/* 辅助识别模块类别 */
#define EBYTE_E22_NAME_TYPE 0x22

/* 辅助识别模块频段 */
#if defined(EBYTE_E22_400M22S)
#define EBYTE_E22_FREQUENCY_TYPE 0x400 
#elif defined(EBYTE_E22_900M22S)
#define EBYTE_E22_FREQUENCY_TYPE 0x900 

#endif 

/* 辅助识别驱动程序版本号 */
#define EBYTE_E22_PROGRAM_TYPE 0x10 
...
#if ( RF_TX_RX_MODE == 0) 
/* 接收: LoRa速率查照表 */
static const int8e_t Ebyte_E22x_LoraBpsTable[8][3] =
{
 ...
 * @note  高电平(外部IO)表示 忙
 */
void Ebyte_E22x_WaitOnBusy( void )
{
 ...
 */
RadioOperatingModes_t Ebyte_E22x_GetOperatingMode( void )
{
 ...
 */
void Ebyte_E22x_CheckReady ( void )
{
    if ( ( Ebyte_E22x_GetOperatingMode () == MODE_SLEEP ) || ( Ebyte_E22x_GetOperatingMode () == MODE_RX_DC ) )
    {
        Ebyte_E22x_Wakeup ();
    }
    Ebyte_E22x_WaitOnBusy ();
...
/* !
 * @brief 向E22x模块自由写入指令 并写入数据
 *
 * @param command 支持的指令 请参见 RadioCommands_t定义
 * @param buffer  指令附加数据首指针
 * @param size    指令附加数据的长度
 */
void Ebyte_E22x_WriteCommand( uint8e_t command, uint8e_t *buffer, uint16e_t size )
{

    /* 当前状态标识 */
    Ebyte_E22x_CheckReady( );
...
/* !
 * @brief 向E22x模块自由写入指令 并读取数据
 *
 * @param command 支持的指令 请参见 RadioCommands_t定义
 * @param buffer  指针 读出数据将存储在目标地址
 * @param size    需要读取的数据长度
 */
void Ebyte_E22x_ReadCommand( uint8e_t command, uint8e_t *buffer, uint16e_t size )
{
    /* 当前状态标识 */
    Ebyte_E22x_CheckReady( );
 ...
 * @param size    指令附加数据的长度
 */
void Ebyte_E22x_WriteRegisters( uint16e_t address, uint8e_t *buffer, uint16e_t size )
{
    /* 当前状态 */
    Ebyte_E22x_CheckReady( );
 ...
 * @param value   待写入值
 */
void Ebyte_E22x_WriteRegister( uint16e_t address, uint8e_t value )
{
    Ebyte_E22x_WriteRegisters( address, &value, 1 );
}

/* !
 * @brief 批量读取E22x内部寄存器数值
 *
 * @param address 寄存器开始地址
 * @param buffer  指针 读出数据将存储在目标地址
 * @param size    需要读取的数据长度
 */
void Ebyte_E22x_ReadRegisters( uint16e_t address, uint8e_t *buffer, uint16e_t size )
{
    /* 当前状态 */
    Ebyte_E22x_CheckReady( );
...
/* !
 * @brief  读取E22x内部某个寄存器数值
 *
 * @param  address 目标寄存器地址
 * @return 寄存器值
 */
uint8e_t Ebyte_E22x_ReadRegister( uint16e_t address )
{
    uint8e_t data;
    Ebyte_E22x_ReadRegisters( address, &data, 1 );
    return data;
}

/* !
 * @brief 批量写入E22x内部 FIFO
 *
 * @param offset  开始地址
 * @param buffer  指向待写入数据
 * @param size    写入的数据长度
 */
void Ebyte_E22x_WriteBuffer( uint8e_t offset, uint8e_t *buffer, uint8e_t size )
{
    /* 当前状态 */
    Ebyte_E22x_CheckReady( );
...
/* !
 * @brief 批量读取E22x内部 FIFO
 *
 * @param offset  开始地址
 * @param buffer  指向待存储数据区域
 * @param size    读取的数据长度
 */
void Ebyte_E22x_ReadBuffer( uint8e_t offset, uint8e_t *buffer, uint8e_t size )
{
    /* 当前状态 */
    Ebyte_E22x_CheckReady( );
 ...
 * @brief 控制模块IO 进入发送模式
 */
void Ebyte_E22x_RfIoEnterTxMode( void )
{
    /* IO RXEN  */
 ...
 * @brief 控制模块IO 进入接收模式
 */
void Ebyte_E22x_RfIoEnterRxMode( void )
{
    /* IO TXEN  */
...
/* !
 * @brief 设置E22x模块的待机时钟模式
 *
 * @param  standbyConfig 待机类型
 *         @arg STDBY_RC: 即内部的RC低频时钟，可以降低功耗
 *         @arg STDBY_XOSC：只有在应用程序对时钟要求严格，我们才需要打开XOSX模块
 */
void Ebyte_E22x_SetStandby( RadioStandbyModes_t standbyConfig )
{
    Ebyte_E22x_WriteCommand( RADIO_SET_STANDBY, ( uint8e_t* )&standbyConfig, 1 );
 ...
 */
void Ebyte_E22x_Calibrate( CalibrationParams_t calibParam )
{
    Ebyte_E22x_WriteCommand( RADIO_CALIBRATE, ( uint8e_t* )&calibParam, 1 );
}
 ...
 *         1;开启
 */
void Ebyte_E22x_SetDio2AsRfSwitchCtrl( uint8e_t enable )
{
    Ebyte_E22x_WriteCommand( RADIO_SET_RFSWITCHMODE, &enable, 1 );
}

/* !
 * @brief 内部状态机 用于统一识别收发调试方式 LoRa / GFSK
 */
RadioPacketTypes_t Ebyte_E22x_GetPacketType( void )
{
    return PacketType;
}
 ...
 */
void Ebyte_E22x_GetRxBufferStatus( uint8e_t *payloadLength, uint8e_t *rxStartBufferPointer )
{
    uint8e_t status[2];

    Ebyte_E22x_ReadCommand( RADIO_GET_RXBUFFERSTATUS, status, 2 );

    // In case of LORA fixed header, the payloadLength is obtained by reading
    // the register REG_LR_PAYLOADLENGTH
//    if( ( Ebyte_E22x_GetPacketType( ) == PACKET_TYPE_LORA ) && ( Ebyte_E22x_ReadRegister( REG_LR_PACKETPARAMS ) >> 7 == 1 ) )
    if( ( Ebyte_E22x_ReadRegister( REG_LR_PACKETPARAMS ) >> 7 == 1 ) )
    {
        *payloadLength = Ebyte_E22x_ReadRegister( REG_LR_PAYLOADLENGTH );
    }
    else
    {
 ...
 * @param size 待发送数据长度
 */
void Ebyte_E22x_SetPayload( uint8e_t *payload, uint8e_t size )
{
    Ebyte_E22x_WriteBuffer( 0x00, payload, size );
}
 ...
 * @param size   读取长度 
 * @param maxSize 最大接收长度 关联接收缓存 见头文件配置 RX_BUFFER_SIZE
 */
uint8e_t Ebyte_E22x_GetPayload( uint8e_t *buffer, uint8e_t *size,  uint8e_t maxSize )
{
    uint8e_t offset = 0;

    Ebyte_E22x_GetRxBufferStatus( size, &offset );
    if( *size > maxSize )
    {
        return 1;
    }
    Ebyte_E22x_ReadBuffer( offset, buffer, *size );
    return 0;
}
 ...
 */
void Ebyte_E22x_SetDioIrqParams( uint16e_t irqMask, uint16e_t dio1Mask, uint16e_t dio2Mask, uint16e_t dio3Mask )
{
    uint8e_t buf[8];
    ...
    Ebyte_E22x_WriteCommand( RADIO_CFG_DIOIRQ, buf, 8 );
}
 ...
 */
void Ebyte_E22x_ClearIrqStatus( uint16e_t irq )
{
    uint8e_t buf[2];

    buf[0] = ( uint8e_t )( ( ( uint16e_t )irq >> 8 ) & 0x00FF );
    buf[1] = ( uint8e_t )( ( uint16e_t )irq & 0x00FF );
    Ebyte_E22x_WriteCommand( RADIO_CLR_IRQSTATUS, buf, 2 );
}
 ...
 *                @arg 0:不进行超时判断
 */
void Ebyte_E22x_SetTx( uint32e_t timeout )
{
    uint8e_t buf[3];

    OperatingMode = MODE_TX;

    Ebyte_E22x_SetDioIrqParams ( IRQ_TX_DONE,
                                 IRQ_TX_DONE, //兼容 硬件DIO1-IRQ中断模式
                                 IRQ_RADIO_NONE,
                                 IRQ_RADIO_NONE );

    Ebyte_E22x_ClearIrqStatus ( IRQ_TX_DONE );

    /* TXEN引脚 切换电路进入发送模式 */
    Ebyte_E22x_RfIoEnterTxMode();

    buf[0] = ( uint8e_t )( ( timeout >> 16 ) & 0xFF );
    buf[1] = ( uint8e_t )( ( timeout >> 8 ) & 0xFF );
    buf[2] = ( uint8e_t )( timeout & 0xFF );
    Ebyte_E22x_WriteCommand( RADIO_SET_TX, buf, 3 );
}

/* !
 * @brief LoRa发送数据包框架(前导码等)参数配置
 * @param size 消息体的长度
 * @note  注意时序，必须在 Ebyte_E22x_SetPacketType()函数设定LORA调制模式之后调用
 *        头文件中可自定义参数
 */
void Ebyte_E22x_SetLoraPacketParams( uint8e_t size )
{
    uint8e_t buf[9] = { 0 };
    ...
    /* 前导码写入模块 */
    Ebyte_E22x_WriteCommand( RADIO_SET_PACKETPARAMS, buf, 6 );
}
 ...
 * @param seed CRC参数
 */
void Ebyte_E22x_SetCrcSeed( uint16e_t seed )
{
    uint8e_t buf[2];
    ...
    switch( Ebyte_E22x_GetPacketType( ) )
    {
    case PACKET_TYPE_GFSK:
        Ebyte_E22x_WriteRegisters( REG_LR_CRCSEEDBASEADDR, buf, 2 );
        break;
 ...
 */
void Ebyte_E22x_SetCrcPolynomial( uint16e_t polynomial )
{
    uint8e_t buf[2];
    ...
    {
    case PACKET_TYPE_GFSK:
        Ebyte_E22x_WriteRegisters( REG_LR_CRCPOLYBASEADDR, buf, 2 );
        break;
...
void Ebyte_E22x_SetFskPacketParams( uint8e_t size )
{
    uint8e_t buf[9] = { 0 };  
    uint8e_t crcVal = 0;
    uint16e_t preambleLength = 0;

    Ebyte_E22x_SetCrcSeed( CRC_CCITT_SEED );
    Ebyte_E22x_SetCrcPolynomial( CRC_POLYNOMIAL_CCITT );
    ...
    Ebyte_E22x_WriteCommand( RADIO_SET_PACKETPARAMS, buf, 9 );
}
 ...
 *                @arg 0:不进行超时判断
 */
void Ebyte_E22x_SetRx( uint32e_t timeout )
{
    uint8e_t buf[3];
    ...
    /* 启用模块接收中断 */
    Ebyte_E22x_SetDioIrqParams ( IRQ_RX_DONE,
                                 IRQ_RX_DONE,//兼容 硬件DIO1-IRQ中断模式
                                 IRQ_RADIO_NONE,
                                 IRQ_RADIO_NONE);    
    
#if ( RF_TX_RX_MODE == 0)         
    Ebyte_E22x_SetLoraPacketParams(0xFF);
#else
    Ebyte_E22x_SetFskPacketParams(0xFF);
#endif    
    
    /* 清除中断标识 */
    Ebyte_E22x_ClearIrqStatus ( IRQ_RX_DONE );

    /* RXEN IO使能 */
    Ebyte_E22x_RfIoEnterRxMode();

    /* 使能接收模式 */
    buf[0] = ( uint8e_t )( ( timeout >> 16 ) & 0xFF );
    buf[1] = ( uint8e_t )( ( timeout >> 8 ) & 0xFF );
    buf[2] = ( uint8e_t )( timeout & 0xFF );
    Ebyte_E22x_WriteCommand( RADIO_SET_RX, buf, 3 );
}
 ...
 */
uint16e_t Ebyte_E22x_GetIrqStatus( void )
{
    uint8e_t irqStatus[2];

    Ebyte_E22x_ReadCommand( RADIO_GET_IRQSTATUS, irqStatus, 2 );
 ...
 */
RadioStatus_t Ebyte_E22x_GetStatus( void )
{
    uint8e_t stat = 0;
    RadioStatus_t status;

    Ebyte_E22x_ReadCommand( RADIO_GET_STATUS, ( uint8e_t * )&stat, 1 );
    status.Value = stat;
    return status;
}
 ...
 */
RadioError_t Ebyte_E22x_GetDeviceErrors( void )
{
    RadioError_t error;

    Ebyte_E22x_ReadCommand( RADIO_GET_ERROR, ( uint8e_t * )&error, 2 );
    return error;
}
 ...
 * @param syncWord
 */
void Ebyte_E22x_SetLoraSyncWord( uint16e_t  syncWord )
{
    Ebyte_E22x_WriteRegister( REG_LR_SYNCWORDBASEADDRESS, ( syncWord >> 8 ) & 0xFF  );
    Ebyte_E22x_WriteRegister( REG_LR_SYNCWORDBASEADDRESS + 1,  syncWord  & 0xFF );
}
 ...
 * @param syncWord
 */
void Ebyte_E22x_SetFskSyncWord(void)
{
    ...
    Ebyte_E22x_WriteRegisters( REG_LR_SYNCWORDBASEADDRESS, syncWord, 8 );
}


void Ebyte_E22x_SetWhiteningSeed( uint16e_t seed )
{
    uint8e_t regValue = 0;
    ...
    {
    case PACKET_TYPE_GFSK:
        regValue = Ebyte_E22x_ReadRegister( REG_LR_WHITSEEDBASEADDR_MSB ) & 0xFE;
        regValue = ( ( seed >> 8 ) & 0x01 ) | regValue;
        Ebyte_E22x_WriteRegister( REG_LR_WHITSEEDBASEADDR_MSB, regValue ); // only 1 bit.
        Ebyte_E22x_WriteRegister( REG_LR_WHITSEEDBASEADDR_LSB, ( uint8e_t )seed );
        break;
...
uint32e_t Ebyte_E22x_GetRandom( void )
{
    ...
    Ebyte_E22x_ReadRegisters( RANDOM_NUMBER_GENERATORBASEADDR, buf, 4 );

    Ebyte_E22x_SetStandby( STDBY_RC );
 ...
 */
void Ebyte_E22x_SetSleep( uint8e_t  cmd )
{
    Ebyte_Port_TxenIoControl(0);
    Ebyte_Port_RxenIoControl(0);
    Ebyte_E22x_WriteCommand( RADIO_SET_SLEEP, &cmd, 1 );
 ...
 * @param 仅用于调试
 */
void Ebyte_E22x_SetFs( void )
{
    Ebyte_E22x_WriteCommand( RADIO_SET_FS, 0, 0 );
    OperatingMode = MODE_FS;
}


void Ebyte_E22x_SetRxBoosted( uint32e_t timeout )
{
    uint8e_t buf[3];

    OperatingMode = MODE_RX;

    Ebyte_E22x_WriteRegister( REG_RX_GAIN, 0x96 ); // max LNA gain, increase current by ~2mA for around ~3dB in sensivity
 ...
 */
void Ebyte_E22x_SetRxDutyCycle( uint32e_t rxTime, uint32e_t sleepTime )
{
    uint8e_t buf[6];
    ...
    Ebyte_E22x_WriteCommand( RADIO_SET_RXDUTYCYCLE, buf, 6 );
 ...
 * @note 需要配置信道检测参数
 */
void Ebyte_E22x_SetCad( void )
{
    Ebyte_E22x_WriteCommand( RADIO_SET_CAD, 0, 0 );
 ...
 * @brief 仅用于测试
 */
void Ebyte_E22x_SetTxContinuousWave( void )
{
    Ebyte_E22x_WriteCommand( RADIO_SET_TXCONTINUOUSWAVE, 0, 0 );
}
/* !
 * @brief 仅用于测试
 */
void Ebyte_E22x_SetTxInfinitePreamble( void )
{
    Ebyte_E22x_WriteCommand( RADIO_SET_TXCONTINUOUSPREAMBLE, 0, 0 );
}

void Ebyte_E22x_SetStopRxTimerOnPreambleDetect( uint8e_t enable )
{
    Ebyte_E22x_WriteCommand( RADIO_SET_STOPRXTIMERONPREAMBLE, ( uint8e_t* )&enable, 1 );
}

void Ebyte_E22x_SetLoRaSymbNumTimeout( uint8e_t SymbNum )
{
    Ebyte_E22x_WriteCommand( RADIO_SET_LORASYMBTIMEOUT, &SymbNum, 1 );
}
 ...
 */
void Ebyte_E22x_SetRegulatorMode( RadioRegulatorMode_t mode )
{
    Ebyte_E22x_WriteCommand( RADIO_SET_REGULATORMODE, ( uint8e_t* )&mode, 1 );
}
 ...
 * @param freq 模式
 */
void Ebyte_E22x_CalibrateImage( uint32e_t freq )
{
    uint8e_t calFreq[2];
    ...
    Ebyte_E22x_WriteCommand( RADIO_CALIBRATEIMAGE, calFreq, 2 );
}

/* !
 * @brief 设定PA参数 详情请参考手册
 */
void Ebyte_E22x_SetPaConfig( uint8e_t paDutyCycle, uint8e_t hpMax, uint8e_t deviceSel, uint8e_t paLut )
{
    uint8e_t buf[4];
    ...
    buf[2] = deviceSel;
    buf[3] = paLut;
    Ebyte_E22x_WriteCommand( RADIO_SET_PACONFIG, buf, 4 );
}

void Ebyte_E22x_SetRxTxFallbackMode( uint8e_t fallbackMode )
{
    Ebyte_E22x_WriteCommand( RADIO_SET_TXFALLBACKMODE, &fallbackMode, 1 );
}
 ...
 */
void Ebyte_E22x_SetRfFrequency( uint32e_t frequency )
{
    uint8e_t buf[4];
    uint32e_t freq = 0;

    Ebyte_E22x_CalibrateImage( frequency );

    Ebyte_E22x_SetDio3AsTcxoCtrl ( TCXO_CTRL_3_3V, 320 );
    Ebyte_E22x_SetStandby ( STDBY_XOSC );
    ...
    Ebyte_E22x_WriteCommand( RADIO_SET_RFFREQUENCY, buf, 4 );
}
 ...
 */
void Ebyte_E22x_SetPacketType( RadioPacketTypes_t packetType )
{
    PacketType = packetType;
    Ebyte_E22x_WriteCommand( RADIO_SET_PACKETTYPE, ( uint8e_t* )&packetType, 1 );
}
 ...
 */
void Ebyte_E22x_SetTxPaParams( int8e_t power, RadioRampTimes_t rampTime )
{
    uint8e_t buf[2];

    Ebyte_E22x_SetPaConfig( 0x04, 0x07, 0x00, 0x01 );
    if( power > 22 )
    {
        power = 22;
    }
    ...
    Ebyte_E22x_WriteRegister( REG_OCP, 0x38 ); // current max 160mA for the whole device

    buf[0] = power;
    buf[1] = ( uint8e_t )rampTime;
    Ebyte_E22x_WriteCommand( RADIO_SET_TXPARAMS, buf, 2 );
}
 ...
 */
void Ebyte_E22x_SetModulationLoraParams( RadioLoRaSpreadingFactors_t  datarate, RadioLoRaBandwidths_t bandwidth, RadioLoRaCodingRates_t coderate )
{
    uint8e_t buf[8] = { 0 };
    ...
    buf[3] = 0x01; //低速率优化开启
    /* 写入模块 */
    Ebyte_E22x_WriteCommand( RADIO_SET_MODULATIONPARAMS, buf, 4 );
}
 ...
 * @note fdev > datarate
 */
void Ebyte_E22x_SetModulationFskParams( uint32e_t fdev, uint8e_t bandwidth, RadioModShapings_t shaping, uint32e_t bitrate )
{
    uint32e_t tempVal = 0;
    uint8e_t buf[8] = { 0 };
    ...
    buf[7] = ( tempVal& 0xFF );
    Ebyte_E22x_WriteCommand( RADIO_SET_MODULATIONPARAMS, buf, 8 );
}

void Ebyte_E22x_SetCadParams( RadioLoRaCadSymbols_t cadSymbolNum, uint8e_t cadDetPeak, uint8e_t cadDetMin, RadioCadExitModes_t cadExitMode, uint32e_t cadTimeout )
{
    uint8e_t buf[7];
    ...
    Ebyte_E22x_WriteCommand( RADIO_SET_CADPARAMS, buf, 7 );
    OperatingMode = MODE_CAD;
}

void Ebyte_E22x_SetBufferBaseAddress( uint8e_t txBaseAddress, uint8e_t rxBaseAddress )
{
    uint8e_t buf[2];

    buf[0] = txBaseAddress;
    buf[1] = rxBaseAddress;
    Ebyte_E22x_WriteCommand( RADIO_SET_BUFFERBASEADDRESS, buf, 2 );
}
 ...
 */
uint8e_t Ebyte_E22x_GetFskBandwidthRegValue( uint32e_t bandwidth )
{
    uint8e_t i;
...
int8e_t Ebyte_E22x_GetRssiInst( void )
{
    uint8e_t buf[1];
    int8e_t rssi = 0;

    Ebyte_E22x_ReadCommand( RADIO_GET_RSSIINST, buf, 1 );
    rssi = -buf[0] >> 1;
    return rssi;
}

void Ebyte_E22x_GetFskPacketStatus( PacketStatus_t *pktStatus )
{
    uint8e_t status[3];

    Ebyte_E22x_ReadCommand( RADIO_GET_PACKETSTATUS, status, 3 );  
...
void Ebyte_E22x_GetLoraPacketStatus( PacketStatus_t *pktStatus )
{
    uint8e_t status[3];
...
void Ebyte_E22x_ClearDeviceErrors( void )
{
    uint8e_t buf[2] = { 0x00, 0x00 };
    Ebyte_E22x_WriteCommand( RADIO_CLR_ERROR, buf, 2 );
}
 ...
 * @param timeout 超时时间
 */
void Ebyte_E22x_SendPayload( uint8e_t *payload, uint8e_t size, uint32e_t timeout )
{
  
#if ( RF_TX_RX_MODE == 0)    
    /* LoRa 前导码等帧格式配置 */
    Ebyte_E22x_SetLoraPacketParams( size );
#else
    /* Fsk 前导码等帧格式配置 */
    Ebyte_E22x_SetFskPacketParams( size );
#endif    
    
    /* 将数据写入模块内部FIFO */
    Ebyte_E22x_SetPayload( payload, size );

    /* 模块进入发送状态 开始自主发送数据 完成后内部寄存器置位 */
    Ebyte_E22x_SetTx( timeout );
    ...
    do
    {
        ...
        irqStatus = Ebyte_E22x_GetIrqStatus( );
    }
    while( !irqStatus );
    
    /* 清除内部中断标识 */
    Ebyte_E22x_ClearIrqStatus ( irqStatus );
...
void Ebyte_E22x_Init( void )
{
    ...
    /* 启用TCXO(由DIO3控制) 并尝试进入STDBY_XOSC模式  3.3V输出 PLL会自动补偿33.4pf电容器 */
    Ebyte_E22x_SetDio3AsTcxoCtrl ( TCXO_CTRL_3_3V, 320 );
    Ebyte_E22x_WriteRegister ( REG_XTA_TRIM, 0x2F );

    /* 重新计算修正时钟 */
    CalibrationParams_t calibrationParams;
    calibrationParams.Value = 0xFF;
    Ebyte_E22x_Calibrate( calibrationParams );

    /* 进入 STDBY_XOSC 待机配置模式 */
    Ebyte_E22x_SetStandby ( STDBY_XOSC );

    /* 是否启用 DIO2 自动控制TXEN */
    Ebyte_E22x_SetDio2AsRfSwitchCtrl( 1 );

    /* 选择内部电压调节器模式 高效DC-DC */
    Ebyte_E22x_SetRegulatorMode( USE_DCDC );

    /* 内部FIFO读写地址复位 0x00 */
    Ebyte_E22x_SetBufferBaseAddress( 0x00, 0x00 );

    
#if ( RF_TX_RX_MODE == 0)  
    /* 数据调制方式 LoRa */
    Ebyte_E22x_SetPacketType( PACKET_TYPE_LORA );

    /* LoRa 关键参数 默认2.4K空速 扩频因子(SF11)  BW(500KHz) CR(4/5) */
    RadioLoRaBandwidths_t bw = (RadioLoRaBandwidths_t)(Ebyte_E22x_LoraBpsTable[RF_LORA_AIR_BPS][0]);
    RadioLoRaSpreadingFactors_t sf = (RadioLoRaSpreadingFactors_t)(Ebyte_E22x_LoraBpsTable[RF_LORA_AIR_BPS][1]);
    RadioLoRaCodingRates_t cr = ((RadioLoRaCodingRates_t)Ebyte_E22x_LoraBpsTable[RF_LORA_AIR_BPS][2] );
    Ebyte_E22x_SetModulationLoraParams ( sf,  bw, cr);   
    
    /* LoRa 同步字 */
    Ebyte_E22x_SetLoraSyncWord( LORA_MAC_PUBLIC_SYNCWORD );

    /* LoRa 前导码等帧格式配置 */
    Ebyte_E22x_SetLoraPacketParams( 0xFF );    
#else    
    /* 数据调制方式 GFSK */
    Ebyte_E22x_SetPacketType( PACKET_TYPE_GFSK ); 
    
    /* FSK 关键参数  */
    uint32e_t bw = Ebyte_E22x_GetFskBandwidthRegValue( RF_FSK_BANDWIDTH );
    Ebyte_E22x_SetModulationFskParams( RF_FSK_FREQUENCY_DEVIATION , bw , MOD_SHAPING_G_BT_1, RF_FSK_DATA_RATE);
    
    /* FSK 前导码等帧格式配置 */
    Ebyte_E22x_SetFskPacketParams( 0xFF );
    
    /* FSK 同步字  */
    Ebyte_E22x_SetFskSyncWord();
      
    Ebyte_E22x_SetWhiteningSeed( 0x01FF );
#endif    
    ...
    Ebyte_E22x_SetRfFrequency( fs );

    /* 发射功率dbm  */
    Ebyte_E22x_SetTxPaParams( RF_TX_OUTPUT_POWER, RADIO_RAMP_3400_US );
 ...
 * @brief 保留 待兼容DIO硬件中断
 */
void Ebyte_E22x_InterruptTrigger( void )
{
 ...
 */
void Ebyte_E22x_IntOrPollTask( void )
{
    /* 读取内部状态 */
    uint16e_t irqStatus = Ebyte_E22x_GetIrqStatus( );
    ...
    if( irqStatus != 0 )
    {
        ...
        if( ( irqStatus & IRQ_RX_DONE ) ==  IRQ_RX_DONE )
        {
            /* 获取数据包状态信息 */
#if ( RF_TX_RX_MODE == 0)            
            Ebyte_E22x_GetLoraPacketStatus( &PacketStatus );
#else
            Ebyte_E22x_GetFskPacketStatus( &PacketStatus );
#endif            
            /* 读取数据包*/
            Ebyte_E22x_GetPayload( RecvPacketBuffer, &recvLength, RF_RX_BUFFER_SIZE );
 ...
 */
uint32e_t Ebyte_E22x_GetName(void)
{
 ...
 * @note 例如0x10 代表V1.0
 */
uint8e_t Ebyte_E22x_GetDriverVersion(void)
{
