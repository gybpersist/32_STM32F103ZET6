...
u8 TP_Init(void)
{
  GPIO_InitTypeDef  GPIO_InitStructure;
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOC|RCC_AHB1Periph_GPIOF, ENABLE);	 //使能PB,PF端口时钟
		
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1|GPIO_Pin_2;				 // PB1端口配置
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;				 // PB2端口配置
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13;				 // PB2端口配置
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//输入
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
  GPIO_Init(GPIOC, &GPIO_InitStructure);//初始化 //PB15 推挽输出,控制背光
	GPIO_SetBits(GPIOC,GPIO_Pin_0);//上拉		
		
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;				 // F9，PF11端口配置
