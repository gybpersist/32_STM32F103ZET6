...
static void Int_NorFlash_WaitNotBusy(void)
{
    ...
    {
        Dri_SPI_Start();
        Dri_SPI_SwapByte(0x05); // 读取状态寄存器指令
...
static void Int_NorFlash_EraseSector(uint8_t block_addr, uint8_t sector_addr)
{
    ...
    // 3 发送扇区擦除指令
    Dri_SPI_Start();
    Dri_SPI_SwapByte(0x20); // 扇区擦除指令
    Dri_SPI_SwapByte(block_addr);                   //块地址
    Dri_SPI_SwapByte((sector_addr << 4) | 0x00);    //扇区地址+页地址
    Dri_SPI_SwapByte(0x00);                         //字节地址
    Dri_SPI_Stop();
...
void Int_NorFlash_GetProductID(uint8_t *manu_id, uint16_t *product_id)
{
    ...
    // 3 接收厂商编号
    *manu_id = Dri_SPI_SwapByte(0x00);
    // 4 接收产品 ID
    *product_id = Dri_SPI_SwapByte(0x00) << 8;
    *product_id |= Dri_SPI_SwapByte(0x00);
...
void Int_NorFlash_WriteData(uint8_t block_addr, uint8_t sector_addr, uint8_t page_addr, uint8_t *data, uint16_t data_len)
{
    ...
    // 3 发送页写入指令
    Dri_SPI_Start();
    Dri_SPI_SwapByte(0x02); // 页写入指令
    Dri_SPI_SwapByte(block_addr);                   //块地址
    Dri_SPI_SwapByte((sector_addr << 4) | (page_addr & 0x0F)); //扇区地址+页地址
    Dri_SPI_SwapByte(0x00);                         //字节地址
